/**
 * generate-throne-scene-2k.mjs
 *
 * Re-renders the throne+Sejong scene at 2K resolution.
 * Reuses the v2 prompt with two model paths:
 *   1. gemini-3-pro-image-preview (Nano Banana Pro) — :generateContent + imageConfig.imageSize='2K'
 *   2. imagen-4.0-ultra-generate-001 — :predict + parameters.sampleImageSize='2K'
 */
...
async function generateNanoBananaPro2K(outName) {
  const model = 'gemini-3-pro-image-preview';
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${KEY}`;
  console.log(`[gen] ${model} @ 2K via imageConfig.imageSize ...`);
  const t0 = Date.now();
  ...
  if (!res.ok) {
    ...
    console.error(`[gen] ${model} 2K FAILED in ${dt}s:`, err.slice(0, 600));
    return null;
  }
  ...
  if (!imagePart) {
    console.error(`[gen] ${model} 2K: no image part`);
    return null;
  }
  ...
  console.log(`[gen] ${model} 2K OK in ${dt}s — ${(buf.length / 1024).toFixed(1)} KB → ${outPath}`);
  return outPath;
}

async function generateImagen4Ultra2K(outName) {
  const model = 'imagen-4.0-ultra-generate-001';
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${model}:predict?key=${KEY}`;
  console.log(`[gen] ${model} @ 2K via :predict ...`);
  const t0 = Date.now();
  ...
  if (!b64) {
    console.error(`[gen] ${model} 2K: no image bytes; raw:`, JSON.stringify(json).slice(0, 600));
    return null;
  }
...
const results = [];
results.push(await generateNanoBananaPro2K('throne-scene-nano-banana-pro-2k.png'));
results.push(await generateImagen4Ultra2K('throne-scene-imagen-4-ultra-2k.png'));

const ok = results.filter(Boolean);
console.log(`\nGenerated ${ok.length}/2 candidates @ 2K.`);
